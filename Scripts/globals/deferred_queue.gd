...
var deferred_update_loops_call: Array[Callable] = []

var queue_dirty: bool = false


func queue_all_turnaround_loop_calculations() -> void:
...
func queue_calculate_turnaround(train: Train) -> void:
	...
	queue_dirty = true
	train.update_loops_dirty = true
	deferred_update_loops_call.append(func() -> void: Graph.update_turnaround_loops_for_train(train))
	call_deferred("process_queue")
	

# Add/remove/delte track
func network_updated() -> void:
	queue_all_turnaround_loop_calculations()
...
func process_queue() -> void:
	...
	##  QUEUE ORDERING ##
	if (!deferred_update_loops_call.is_empty()):
		for callable: Callable in deferred_update_loops_call:
			callable.call()
	...
	deferred_update_loops_call.clear()