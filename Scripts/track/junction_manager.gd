...
func split_track_at_point(trackPointInfo: TrackPointInfo) -> Junction:	
	...
	var first_half: Track = split_tracks[0]
	var second_half: Track = split_tracks[1]
...
func create_split_track(trackPointInfo: TrackPointInfo) -> Array[Track]:
	...
	for i: int in range(new_dubins_paths.size()):
		var newTrack: Track = Track.new_Track("SplitTrack_" + str(TrackBuilder.track_counter), curve_type_flag, track.tracks)
