...
func _init(mouse_tracker_node: MouseTracker) -> void:
	space_state = 	mouse_tracker_node.get_world_2d().direct_space_state
	mouse_tracker = mouse_tracker_node
...
func check_for_overlaps_at_position(position: Vector2) -> TrackPointInfo:
	track_intersection = null
	var results = check_for_collision(position, 1)
	...
	if track_intersection:
		mouse_tracker.draw_circle_at_point(track_intersection.get_point())
